...
void menuGerirMarcacoes(std::vector<Colaborador>& lista) {
    ...
    // Lê a opção primeiro
    if (!(std::cin >> op)) {
        std::cout << COR_VERMELHA << "ERRO: Input invalido para Opcao.\n" << RESET_COR;
        std::cin.clear();
    ...
    if (op < 1 || op > 3) {
        std::cout << COR_VERMELHA << "Opcao invalida.\n" << RESET_COR;
        std::cin.ignore((std::numeric_limits<std::streamsize>::max)(), '\n');
        return;
    }

    std::cout << "Digite a data (Dia Mes Ano): ";
    if (!(std::cin >> dia >> mes >> ano)) {
        std::cout << COR_VERMELHA << "ERRO: Input invalido para Data.\n" << RESET_COR;
        std::cin.clear();
        std::cin.ignore((std::numeric_limits<std::streamsize>::max)(), '\n');
        return;
    }
